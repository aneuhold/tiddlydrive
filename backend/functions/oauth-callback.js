...
exports.handler = async (event) => {
  try {
    ...
    if (!cookiePayload) {
      return { statusCode: 400, body: "Missing td2_oauth cookie (PKCE/state not set or wrong path)" };
    }
    if (!codeVerifier) {
    ...
    const rtCookie = `td2_rt=${enc}; Path=/api/; HttpOnly${secureAttr}; SameSite=Lax; Max-Age=${
      60 * 60 * 24 * 30
    }`;
